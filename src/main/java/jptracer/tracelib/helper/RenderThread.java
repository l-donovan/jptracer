        ...
        while (running.get()) {
            try {
                ...
                for (int x = 0; x < camera.res_x; x++) {
                    ...
                    Vec3 vec = Core.rot(new Vec3(
                        (2 * (x + 0.5) / camera.res_x - 1) * camera.aspectRatio * camera.scale,
                        (1 - 2 * (y + 0.5) / camera.res_y) * camera.scale,
                        1.0
                    ...
                    for (int i = 0; i < options.sampleCount; i++) {
                        Vec3 colorSample = Core.castRay(camera.pos, vec, scene, options, 0);
